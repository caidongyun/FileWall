...
UINT32 GetHash(__in UNICODE_STRING *pStr)
{
	unsigned int	hash = 0;
	int				i    = 0;
	PWSTR			str	 = pStr->Buffer;

	for(i = 0; i < (pStr->Length/2); str++, i++)
	{
...
NTSTATUS ApRequestAccess(__in PACCESS_DATA pAccessData)
{
	...
	PREPLY_DATA		pReplyData	= NULL;	// Data to receive from application.

	PAGED_CODE();

	if(pAccessData != NULL)
	{
		pReplyData = (PREPLY_DATA)ExAllocatePoolWithTag(NonPagedPool,
			sizeof(REPLY_DATA),
			POOL_TAG);
		...
		if(pReplyData != NULL)
		{
			...
			status = FltSendMessage(g_FilterHandle,	// Filter handle.
				&g_hClientPort,						// Communication port handle.
				pAccessData,						// ACCESS_DATA struct passed to application.
				sizeof(ACCESS_DATA),				// Size of ACCESS_DATA.
				pReplyData,							// Reply from application.
				&ReplyDataSize,						// Size of reply struct.
				NULL);								// Timeout.

			// Now we waiting response from application.
	...
	if(pAccessData != NULL)
		ExFreePool(pAccessData);
	if(pReplyData != NULL)
		ExFreePoolWithTag(pReplyData, POOL_TAG);

	// Print info if failed.
	// Don't print any if STATUS_ACCESS_DENIED.
	if(NT_SUCCESS(status) == FALSE && status != STATUS_ACCESS_DENIED)
	{
		KdPrint(("ApRequestAccess failed. Status = %x\n", status));
...
// Returns NULL if specified path is not under protect.
// Otherwise returns pointer to AP_PROTECTED_ENTRY struct.
									 ...
									 __in	UNICODE_STRING		*pName)
{
	...
	PAP_PROTECTED_ENTRY pEntryAdded = NULL;

	//KdPrint(("ApIsUnderProtect: %wZ\n", pName));
	
	Entry.Hash = GetHash(pName);

	pEntryAdded = (PAP_PROTECTED_ENTRY)RtlLookupElementGenericTable(pGenericTable, &Entry);
...
PCHAR Status2String(__in NTSTATUS status)
{
	...
	case 0x1A:
		return "FACILITY_COMMONLOG_ERROR_CODE";
